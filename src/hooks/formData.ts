...
export function objectToFormData<TForm extends FormDataType>(data: TForm, parentKey: string | null = null): FormData {
  const form = new FormData();

  for (const key in data) {
    if (Object.prototype.hasOwnProperty.call(data, key)) {
      append(form, composeKey(parentKey, key), data[key] as FormDataConvertible)
    }
  }
